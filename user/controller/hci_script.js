	$(document).ready(function(){
		...
		$("#add_row").click(function(){
			if ($(".uid"+(i-1)).val() == '' || $(".uname"+(i-1)).val() == '') {
				...
				$('#hci_tab_logic').append("<tr id='addr"+i+"'><td></td><td><input class='form-control text-dark input-md uid"+i+"' type='text' name='others_1[]'  /></td><td><input class='form-control text-dark input-md uname"+i+"' type='text' name='others_2[]'  /></td></tr>");
				i++; 				
			}
		});
    ...
    $(document).ready(function(){
                ...
                success: function (data) {
                    ...
                    $.each(data, function (key, value) {
                        if (value.hostname.search(expression) != -1) {
                            $('#hci_up_search_result').append('<li class="list-group-item list-group-item-action" id="' + value.hostname + '" style="cursor: pointer;"><span class="font-weight-bold sp_destination" id="' + value.hostname + '">' + value.hostname + '</span></li>');
                        }
                    });
                }
            });
        ...
        $('#hci_up_search_txt').keyup(function () {
