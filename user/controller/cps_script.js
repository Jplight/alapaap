$(document).ready(function () {
	...
	$("#add_row_cps").click(function () {
		if (
			$(".cps_disk_req" + (i - 1)).val() == "" ||
			$(".cps_disk_info" + (i - 1)).val() == "" ||
			$(".cps_disk_remarks" + (i - 1)).val() == ""
		) {
			alert("Please Fill up the fields!" + (i - 1))
		} else {
			$("#cps_tab_logic").append(
				"<tr id='addr_cps" +
					i +
					"'><td></td><td><input class='form-control text-dark input-md cps_disk_req" +
					i +
					"' type='text' name='others_1[]'  /></td><td><input class='form-control text-dark input-md cps_disk_info" +
					i +
					"' type='text' name='others_2[]'  /></td><td><input class='form-control text-dark cps_disk_remarks" +
					i +
					"' type='text' name='others_3[]' ></td></tr>"
			)
			i++
		}
	})
